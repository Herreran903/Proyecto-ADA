...
class ArbolRojoNegro:
    ...
    def suma_arbol(self, nodo):
        if nodo == self.NIL:
            return 0  # Suma inicializada en cero para el caso base

        # Recorre recursivamente los subárboles izquierdo y derecho
        suma_izquierda = self.suma_arbol(nodo.izquierda)
        suma_derecha = self.suma_arbol(nodo.derecha)

        # Calcula la suma total
        suma_total = suma_izquierda + suma_derecha + nodo.clave

        return suma_total

    def suma(self):
        suma_total = self.suma_arbol(self.raiz)
        return suma_total
